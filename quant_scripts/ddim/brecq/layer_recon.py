...
from quant_scripts.ddim.brecq.data_utils import save_grad_data, save_inp_oup_data
...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_x, cali_t, 
    ...
    loss_func = LossFunction(layer, round_loss=loss_mode, weight=weight,
                             ...
                             decay_start=0, warmup=warmup, p=p, device=device)

    # Save data before optimizing the rounding
    cached_inps_trajs, cached_outs_trajs, cached_grads_trajs = [], [], []
    time_steps = len(cali_x)
    cali_samples_num = cali_x[0].shape[0]
    for i in range(time_steps):
        cached_inps, cached_outs = save_inp_oup_data(model, layer, cali_x[i], cali_t[i], asym, act_quant, batch_size)
        cached_inps_trajs.append(cached_inps)
        cached_outs_trajs.append(cached_outs)
    if opt_mode != 'mse' and opt_mode != 'traj_align':
        for i in range(time_steps):
            cached_grads = save_grad_data(model, layer, cali_x[i], cali_t[i], act_quant, batch_size=batch_size)
    ...
    for i in range(iters):
        ...
        for j in range(time_steps):
            cur_inp = cached_inps_trajs[j][idx].to(device)
        ...
        err = loss_func(out_quant_trajs, cur_out_trajs, cur_grad_trajs)
